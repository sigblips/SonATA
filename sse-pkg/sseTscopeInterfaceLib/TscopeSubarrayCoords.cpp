...
TscopeSubarrayCoords::TscopeSubarrayCoords() 
{
   fill_n(subarray, MAX_TEXT_STRING, '\0');
}

void TscopeSubarrayCoords::marshall()
{
   // no need to marshall subarray char array
   pointing.marshall();
}

void TscopeSubarrayCoords::demarshall()
{
    marshall();
}

ostream& operator << (ostream& strm, const TscopeSubarrayCoords& coords)
{
    ...
    localstrm << "tscope subarray coords:" << endl;
    localstrm << "  subarray: " 
              << coords.subarray << endl;
