...
TscopeSubarray::TscopeSubarray() 
{
   fill_n(subarray, MAX_TEXT_STRING, '\0');
}

void TscopeSubarray::marshall()
{
   // no need to marshall subarray char array
}

void TscopeSubarray::demarshall()
{
    marshall();
}

ostream& operator << (ostream& strm, const TscopeSubarray& sub)
{
    stringstream localstrm;

    localstrm.precision(PrintPrecision);  

    localstrm << "tscope subarray: "
              << sub.subarray << endl;
